...
import { submitCandidature, fetchUserCandidatures, updateProfile, fetchUserPayments, fetchProfile } from "../store/userSlice";
import Header from "../components/Header";

export default function Dossier() {
    const navigate = useNavigate();
    const dispatch = useDispatch();
    const { profile, candidatures, payments, loading, error } = useSelector((state) => state.user);
    ...
    useEffect(() => {
        if (profile) {
            setEditData({
                ...
                nationalite: profile.nationalite || 'Togolaise',
                profession: profile.profession || '',
                garant_nom: profile.garant?.nom || '',
                garant_lien: profile.garant?.lien || '',
                garant_telephone: profile.garant?.telephone || '',
                garant_email: profile.garant?.email || '',
                garant_profession: profile.garant?.profession || '',
                garant_revenus: profile.garant?.revenus || '',
            });
        }
    }, [profile]);
                            ...
                            {activeTab === 'personal' && (
                                ...
                                >
                                    <div className="flex justify-between items-center">
                                        ...
                                        {!isEditing ? (
                                            <button 
                                                ...
                                                className="text-[#FF385C] font-bold hover:underline text-sm"
                                            >
                                                Modifier
                                            </button>
                                        ) : (
                                            <div className="flex gap-4">
                                                <button 
                                                    ...
                                                    className="text-gray-500 font-bold hover:underline text-sm"
                                                >
                                                    Annuler
                                                </button>
                                                <button 
                                    ...
                                    {uploadStatus && isEditing && (
                                        <div className={`p-4 rounded-2xl text-sm font-bold flex items-center gap-3 ${uploadStatus.type === 'success' ? 'bg-green-50 text-green-600' : 'bg-red-50 text-red-600'
                                            }`}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-[#b0b0b0] uppercase tracking-widest">Nom</p>
                                            {isEditing ? (
                                                <input
                                                    type="text"
                                                    name="nom"
                                                    ...
                                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2 text-sm font-bold focus:ring-2 focus:ring-[#FF385C]/20 outline-none"
                                                />
                                            ) : (
                                                <p className="font-bold text-[#222222]">{profile?.nom || 'Non renseigné'}</p>
                                            )}
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-[#b0b0b0] uppercase tracking-widest">Prénom</p>
                                            {isEditing ? (
                                                <input
                                                    type="text"
                                                    name="prenom"
                                                ...
                                                <p className="font-bold text-[#222222]">{profile?.prenom || 'Non renseigné'}</p>
                                            )}
                                        </div>
                                        <div className="space-y-1">
                                            <p className="text-[10px] font-bold text-[#b0b0b0] uppercase tracking-widest">Email</p>
                                            <p className="font-bold text-[#717171]">{profile?.email}</p>
                                        </div>
                                        <div className="space-y-1">
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            <p className="font-bold text-[#717171]">{profile?.role}</p>
                                        </div>
                                        <div className="space-y-1">
                                            ...
                                            {isEditing ? (
                                                ...
                                                <p className="font-bold text-[#222222]">{profile?.profession || 'Non renseigné'}</p>
                                            )}
                                        </div>
                                        <div className="space-y-1">
                                    ...
                                    <div className="bg-white rounded-[32px] border border-[#EBEBEB] shadow-xl p-8 md:p-10">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="relative group">
                                                        ...
                                                        <label
                                                            htmlFor="cni-upload"
                                                            className={`flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-[24px] cursor-pointer transition-all ${files.cni ? 'border-green-500 bg-green-50/30' : 'border-[#EBEBEB] hover:border-[#FF385C] hover:bg-[#F9FAFB]'
                                                                }`}
                                                        >
                                                            {files.cni ? (
                                                                <>
                                                ...
                                                <div className="space-y-3">
                                                    ...
                                                    <div className="relative group">
                                                        ...
                                                        <label
                                                            htmlFor="paie-upload"
                                                            className={`flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-[24px] cursor-pointer transition-all ${files.fiche_paie ? 'border-green-500 bg-green-50/30' : 'border-[#EBEBEB] hover:border-[#FF385C] hover:bg-[#F9FAFB]'
                                                                }`}
                                                        >
                                                            {files.fiche_paie ? (
                                                                <>
                                        ...
                                        <div className="mt-12 space-y-8">
                                            <div className="space-y-4">
                                                <h3 className="text-lg font-bold text-[#222222]">Mes quittances</h3>
                                                {payments.filter(p => p.quittance).length === 0 ? (
                                                    <p className="text-sm text-[#717171]">Aucune quittance disponible.</p>
                                                ) : (
                                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                                        {payments.filter(p => p.quittance).map((p) => (
                                                            <div key={p.id} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-[#EBEBEB] hover:shadow-md transition-all group">
                                                                <div className="flex items-center gap-4">
                                                                    <div className="w-10 h-10 bg-gray-50 rounded-xl flex items-center justify-center text-gray-400 group-hover:bg-[#FF385C]/10 group-hover:text-[#FF385C] transition-colors">
                                                                        <FileText size={20} />
                                                                    </div>
                                                                    <div>
                                                                        <p className="text-sm font-bold text-[#222222]">Quittance {new Date(p.date_paiement).toLocaleDateString('fr-FR', { month: 'long', year: 'numeric' })}</p>
                                                                        <p className="text-[10px] text-[#717171]">{new Date(p.date_paiement).toLocaleDateString()}</p>
                                                                    </div>
                                                                </div>
                                                                <a 
                                                                    href={p.quittance.fichier_pdf.startsWith('http') ? p.quittance.fichier_pdf : `http://localhost:8000${p.quittance.fichier_pdf}`}
                                                                    target="_blank"
                                                                    rel="noopener noreferrer"
                                                                    className="p-2.5 bg-gray-50 text-gray-400 hover:bg-[#FF385C] hover:text-white rounded-xl transition-all shadow-sm"
                                                                    download
                                                                >
                                                                    <FileDown size={18} />
                                                                </a>
                                                ...
                                                {candidatures.length === 0 ? (
                                                    ...
                                                    <div className="space-y-4">
                                                        {candidatures.map((cand) => (
                                                            <div key={cand.id} className="flex items-center justify-between p-4 bg-white rounded-2xl border border-[#EBEBEB] hover:shadow-md transition-all">
                                                                <div className="flex items-center gap-4">
                                                                    <img
                                                                        src={cand.bien_photo || "/outils/M1.png"}
                                                                        alt=""
                                                                        className="w-12 h-12 rounded-xl object-cover border border-gray-100"
                                                                    />
                                                                    <div>
                                                                        <p className="text-[10px] font-bold text-[#b0b0b0] uppercase tracking-widest">{cand.statut_display || cand.statut}</p>
                                                                        <p className="text-sm font-bold text-[#222222] line-clamp-1">{cand.bien_adresse}</p>
                                                                        <p className="text-[10px] text-[#717171]">{new Date(cand.date_creation).toLocaleDateString()}</p>
                                                                    </div>
                                                                </div>
                                                                <div className={`px-3 py-1.5 rounded-full text-[10px] font-bold ${cand.statut === 'ACCEPTE' ? 'bg-green-100 text-green-600' :
                                                                        cand.statut === 'REFUSE' ? 'bg-red-100 text-red-600' :
                                                                            'bg-orange-50 text-orange-600'
                                                                    }`}>
                                                                    {cand.statut === 'ACCEPTE' ? 'Acceptée' :
                                                                        cand.statut === 'REFUSE' ? 'Refusée' :
                                                                            'En attente'}
                                                                </div>
                                                            </div>
                                                        ))}
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            {isEditing ? (
                                                <input
                                                    ...
                                                    value={editData.garant_telephone}
                                                    onChange={handleInputChange}
                                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2 text-sm font-bold focus:ring-2 focus:ring-[#FF385C]/20 outline-none"
                                                />
                                            ) : (
                                        ...
                                        <div className="space-y-1">
                                            ...
                                            {isEditing ? (
                                                <input
                                                    ...
                                                    value={editData.garant_profession}
                                                    onChange={handleInputChange}
                                                    className="w-full bg-gray-50 border border-gray-100 rounded-xl px-4 py-2 text-sm font-bold focus:ring-2 focus:ring-[#FF385C]/20 outline-none"
                                                />
                                            ) : (

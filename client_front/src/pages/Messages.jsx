...
export default function Messages() {
  ...
  useEffect(() => {
    if (contacts?.length > 0 && !activeChat) {
      // conversations endpoint returns grouped data; pick first contact
      const firstContact = contacts[0];
      setActiveChat(firstContact.id || firstContact.destinataire || firstContact.expediteur);
    }
  }, [contacts, activeChat]);
  ...
  const handleSend = async (e) => {
    ...
    // POST /api/messages/ — Body: { destinataire: int, contenu: string }
    await dispatch(sendMessage({ destinataire: activeChat, contenu: messageText }));
    setMessageText("");
  };
  ...
  const getContactName = (contact) => {
    ...
    if (contact?.other_user_nom) return contact.other_user_nom;
    if (contact?.destinataire_nom) return contact.destinataire_nom;
    if (contact?.expediteur_nom) return contact.expediteur_nom;
    if (contact?.nom && contact?.prenom) return `${contact.prenom} ${contact.nom}`;
    return contact?.username || contact?.email || 'Inconnu';
  };
  ...
  const currentContact = contacts?.find(c =>
    (c.id || c.destinataire || c.expediteur) === activeChat
  );
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-2">
            {!contacts || contacts.length === 0 ? (
              ...
              contacts.map((contact, idx) => {
                const contactId = contact.id || contact.destinataire || contact.expediteur;
                return (
                  <button
                    key={contactId || idx}
                    onClick={() => setActiveChat(contactId)}
                    className={`w-full flex items-center gap-4 p-4 rounded-2xl transition-all ${activeChat === contactId ? 'bg-brand-50 shadow-sm' : 'hover:bg-gray-50'}`}
                  >
                    <div className="relative">
                      <div className={`w-12 h-12 rounded-full flex items-center justify-center font-bold text-white shadow-md ${activeChat === contactId ? 'bg-brand-500' : 'bg-gray-300'}`}>
                    ...
                    <div className="flex-1 text-left min-w-0">
                      <div className="flex justify-between items-start">
                        <p className={`font-bold text-sm truncate ${activeChat === contactId ? 'text-brand-600' : 'text-gray-900'}`}>{getContactName(contact)}</p>
                      </div>
                      {contact.contenu && (
                        <p className="text-xs text-gray-500 truncate font-medium">{contact.contenu}</p>
                      )}
                    </div>
                  </button>
                );
            ...
            <>
              ...
              <div className="p-6 border-b border-gray-50 flex justify-between items-center bg-white/80 backdrop-blur-md sticky top-0 z-10">
                <div className="flex items-center gap-4">
                  <div className="w-12 h-12 rounded-full bg-brand-500 flex items-center justify-center font-bold text-white shadow-md">
                    {getContactInitial(currentContact)}
                  </div>
                  <div>
                    <p className="font-black text-gray-900 tracking-tight">{getContactName(currentContact)}</p>
                    <p className="text-xs font-bold text-brand-500">{currentContact?.other_user_role || currentContact?.role || 'En ligne'}</p>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6 bg-[#F9FAFB]/30">
                {activeConversation?.map((msg) => {
                  // Use 'expediteur' field from swagger (integer ID)
                  const isOwn = msg.expediteur === profile?.id;
                  return (
                    <motion.div
                    ...
                    >
                      <div className={`max-w-[70%] p-4 rounded-[24px] shadow-sm relative ${isOwn
                          ? 'bg-brand-500 text-white rounded-tr-none'
                          : 'bg-white text-gray-800 rounded-tl-none border border-gray-50'
                        }`}>
                        {!isOwn && msg.expediteur_nom && (
                          <p className="text-[10px] font-bold text-brand-400 mb-1">{msg.expediteur_nom}</p>
                        )}
                        {/* Use 'contenu' field from swagger */}

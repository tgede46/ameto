...
export default function Payment() {
    ...
    // Data from navigation state or defaults
    const initialData = location.state || { amount: 150000, propertyId: 1, propertyTitle: "Mon Foyer", bailId: null };
    ...
    const handleFinalSubmit = async () => {
        if (!reference.trim()) {
            setLocalError("Veuillez entrer la référence du paiement (numéro de reçu T-Money / Flooz / Visa).");
            return;
        }

        // POST /api/paiements/ — Body matches swagger Paiement definition:
        // { bail: int, locataire: string, date_paiement: date, montant: decimal, reference: string }
        const today = new Date().toISOString().split('T')[0]; // YYYY-MM-DD

        const paymentData = {
            bail: initialData.bailId || 1,
            locataire: profile?.id ? String(profile.id) : "1",
            date_paiement: today,
            montant: String(amount),
            reference: `${method} - ${reference.trim()}`,
        };

        setLocalError(null);
        const result = await dispatch(submitPayment(paymentData));
        if (submitPayment.fulfilled.match(result)) {
            ...
            if (typeof errorMsg === 'object') {
                ...
                setLocalError(messages || "Une erreur est survenue lors de l'envoi du paiement.");
            } else {
                setLocalError(errorMsg || "Une erreur est survenue lors de l'envoi du paiement.");
            }
        }
    };
            ...
            <main className="max-w-3xl mx-auto px-6 py-12">
                        ...
                        {[1, 2, 3].map((s) => (
                            <div key={s} className="flex flex-col items-center gap-3 w-20">
                                ...
                                <span className={`text-[10px] font-bold uppercase tracking-widest text-center ${step === s ? 'text-[#222222]' : 'text-[#b0b0b0]'}`}>
                                    {s === 1 ? "Méthode" : s === 2 ? "Référence" : "Terminé"}
                                </span>
                            </div>
                        ))}
                    </div>
                ...
                <AnimatePresence mode="wait">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                {paymentMethods.map((m) => (
                                    <button
                                        ...
                                        className={`flex flex-col items-center gap-4 p-5 rounded-[40px] border-2 transition-all active:scale-95 group ${method === m.id
                            ...
                            <div className="p-6 bg-blue-50/50 border border-blue-100 rounded-3xl flex items-start gap-4">
                                ...
                                <div>
                                    ...
                                    <p className="text-xs text-blue-700 leading-relaxed">
                                        {currentMethod.instructions}. Conservez la référence de la transaction.
                                    </p>
                                </div>
                            </div>
                    ...
                    {step === 2 && (
                        ...
                        >
                            <div>
                                <h2 className="text-3xl font-extrabold text-[#222222] mb-2">Référence de paiement</h2>
                                <p className="text-[#717171]">Entrez la référence du reçu de votre transaction ({currentMethod.label}).</p>
                            </div>
                            ...
                            <div className="space-y-4">
                                <label className="text-sm font-bold text-[#222222] uppercase tracking-widest ml-1">Référence / Numéro de reçu</label>
                                <input
                                    type="text"
                                    value={reference}
                                    onChange={(e) => {
                                    ...
                                    placeholder="Ex: TXN-123456789 ou numéro T-Money/Flooz"
                                    className="w-full bg-[#F9FAFB]/50 border-2 border-gray-50 rounded-2xl px-6 py-5 focus:border-[#FF385C] outline-none text-lg font-bold text-[#222222] transition-colors tracking-wide"
                                />
                            </div>

                            {/* Summary */}
                            <div className="bg-[#F9FAFB] rounded-3xl p-6 border border-[#EBEBEB]">
                                <h3 className="text-sm font-bold text-[#222222] uppercase tracking-widest mb-4">Récapitulatif</h3>
                                <div className="space-y-3 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-[#717171]">Bien</span>
                                        <span className="font-bold text-[#222222]">{initialData.propertyTitle}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-[#717171]">Montant</span>
                                        <span className="font-bold text-[#222222]">{new Intl.NumberFormat('fr-FR').format(amount)} FCFA</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-[#717171]">Méthode</span>
                                        <span className="font-bold text-[#222222]">{currentMethod.label}</span>
                                    </div>
                                    <div className="flex justify-between">
                                        <span className="text-[#717171]">Date</span>
                                        <span className="font-bold text-[#222222]">{new Date().toLocaleDateString('fr-FR')}</span>
                                    </div>
                                </div>
                            </div>
